# agent_createur.py
#
# This self-contained script acts as a "Site Builder Agent".
# It contains product data, an HTML template, and the logic
# to generate a complete landing page file for a specific business idea.

def get_product_data():
    """
    Returns a dictionary with the product information for our target product.
    """
    return {
        ...
        "FEATURE_2_TITLE": "Respectueux de votre vie privée",
        "FEATURE_2_TEXT": "Notre outil fonctionne entièrement dans votre navigateur. Aucune donnée n'est envoyée sur nos serveurs.",
        "FEATURE_3_TITLE": "Simple et Efficace",
        "FEATURE_3_TEXT": "Pas de configuration compliquée. Un seul bouton pour un résultat immédiat et de haute qualité."
    }
...
def generate_site():
    """
    The core function of the agent. It gets the data and template,
    fills the placeholders, and writes the final index.html file.
    """
    print("Récupération des données du produit et du template...")
    product_data = get_product_data()
    html_template = get_landing_page_template()

    print("Remplacement des placeholders avec les données du produit...")
    ...
    try:
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(filled_html)
        print("SUCCÈS : Le fichier 'index.html' a été généré avec succès !")
    except IOError as e:
        print(f"ERREUR : Impossible d'écrire le fichier sur le disque. {e}")

def main():
    """
    Main entry point for the Site Builder Agent.
    """
    print("Agent Créateur de Site : Démarrage...")
    generate_site()
    print("Agent Créateur de Site : Terminé.")
